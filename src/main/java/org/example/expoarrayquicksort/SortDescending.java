...
/**
    ...
    private static boolean greater(double v, double w) {
        return v - w > 0;
    }

 **/
private static boolean greter(Comparable v, Comparable w) {
    return v.compareTo(w) > 0; // Changed to greater than for descending sort
}
    ...
    private static void merge(double[] a, double[] aux, int lo, int mid, int hi) {
        ...
        for (int k = lo; k <= hi; k++) {
            ...
            else if (greter(aux[j], aux[i])) a[k] = aux[j++];
            else  a[k] = aux[i++];
        }
